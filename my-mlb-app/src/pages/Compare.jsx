...
function Compare() {
    ...
    const reverseScoreKeys = new Set(["ERA", "BB9", "WHIP"]); // 可以自行擴充
    ...
    <div className="compare-container">
        ...
        {commonYears.length > 0 && (
            <div className="compare-year-select-section">
                <label className="compare-year-select-label">選擇年份：</label>
                <select
        ...
        {selectedYear && playerData1.length > 0 && playerData2.length > 0 && (() => {
            ...
            return (
                <div className="compare-table-section">
                    ...
                    <table className="compare-table">
                        ...
                        <tbody>
                            <tr>
                                <td>{selectedPitcher1?.Name}</td>
                                {noNameKeys.map((key) => (
                                    <td key={key} style={{ fontWeight: compareValue(key, player1[key], player2[key]) === 1 ? 'bold' : 'normal' }}>
                                        {player1[key]}
                                    </td>
                                ))}
                                <td>{player1Score}</td>
                            ...
                            <tr>
                                <td>{selectedPitcher2?.Name}</td>
                                {noNameKeys.map((key) => (
                                    <td key={key} style={{ fontWeight: compareValue(key, player2[key], player1[key]) === 1 ? 'bold' : 'normal' }}>
                                        {player2[key]}
                                    </td>
                                ))}
                                <td>{player2Score}</td>
                    ...
                    <div className="heatmap-section">
                        <h3>{selectedPitcher1.Name} 的熱區圖</h3>
                        <PitcherHeatMap player={player1} />

                        <h3>{selectedPitcher2.Name} 的熱區圖</h3>
